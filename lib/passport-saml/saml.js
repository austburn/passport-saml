...
var KeyInfo = function (key) {
  ...
  this.getKey = function (keyInfo) {
    var cleanKey = this.key;

    cleanKey = cleanKey.replace( /-+BEGIN CERTIFICATE-+\r?\n?/, '' );
    cleanKey = cleanKey.replace( /-+END CERTIFICATE-+\r?\n?/, '' );
...
var createEnvelopedSignature = function (id, xml, localName, cert, privateCert) {
  var sig;

  sig = new xmlCrypto.SignedXml();
  sig.keyInfoProvider = new KeyInfo(cert);
  sig.addReference("//*[local-name()='" + localName + "']", [
                     'http://www.w3.org/2000/09/xmldsig#enveloped-signature',
                     'http://www.w3.org/2001/10/xml-exc-c14n#'
                   ], undefined, id);
  sig.signingKey = privateCert;
...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, callback) {
  ...
  .then(function(){
    ...
    xml = xmlbuilder.create(request).end();

    if (self.options.privateCert) {
      xml = createEnvelopedSignature(id, xml, 'LogoutRequest',
                                     self.options.cert,
                                     self.options.privateCert);
    } 
...
SAML.prototype.generateLogoutRequest = function (req) {
  ...
  if (this.options.privateCert) {
    xml = createEnvelopedSignature(id, xml, 'AuthnRequest',
                                   this.options.cert,
                                   this.options.privateCert);
  } 
