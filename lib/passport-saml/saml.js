...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, callback) {
  ...
  .then(function(){
    ...
    if (self.options.privateCert) {
      ...
      sig.keyInfoProvider = new KeyInfo(self.options.cert);
      sig.addReference("//*[local-name()='AuthnRequest']", ['http://www.w3.org/2000/09/xmldsig#enveloped-signature', 'http://www.w3.org/2001/10/xml-exc-c14n#'], undefined, id);
      sig.signingKey = self.options.privateCert;
      sig.computeSignature(xmlbuilder.create(request).end());
    }

    callback(null, sig.getSignedXml());
  })
  .fail(function(err){
    callback(err);
  })
